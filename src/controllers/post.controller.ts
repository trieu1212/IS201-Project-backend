...
const postController = {
  ...
  //admin
  deletePost: async (req: PostRequest, res: PostResponse) => {
    const { postId } = req.params;
    try {
  ...
  //user
  deletePostByUser: async (req: PostRequest, res: PostResponse) => {
    const { postId } = req.params;
    try {
      await db.Post.destroy({
        where: { id: postId },
  ...
  updatePost: async (req: PostRequest, res: PostResponse) => {
    const { postId } = req.params;
    const { title, description, roomType, price, address, acreage } = req.body;
    try {
